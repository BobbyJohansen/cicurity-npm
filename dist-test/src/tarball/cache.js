...
export function cachePathForIntegrity(integrity) {
...
export async function isCached(integrity) {
 ...
 * integrity format: "sha512-<base64>"
 */
export async function verifyIntegrity(filePath, integrity) {
    const [algorithm, expected] = integrity.split('-');
    if (!algorithm || !expected)
        return false;
    const nodeAlg = algorithm.toLowerCase().replace('sha', 'sha');
 ...
 */
export async function cacheFile(sourcePath, integrity) {
