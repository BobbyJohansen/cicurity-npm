...
export function analyzePublishAnomaly(context) {
    ...
    const versionsInWindow = versionTimes.filter((v) => v.version !== version &&
        ...
        if (lastBeforeWindow) {
            ...
            if (gapDays > 30) {
                findings.push({
                    ...
                    description: `${versionsInWindow.length + 1} versions of '${context.packageName}' were published within ` +
    ...
    if (previousEntry) {
        ...
        if (dormancyMonths >= DORMANCY_THRESHOLD_MONTHS) {
            findings.push({
                ...
                description: `'${context.packageName}' had no new versions for ${Math.round(dormancyMonths)} months ` +
    ...
    if (previousEntry) {
        ...
        if (prevMajor !== null && currMajor !== null && currMajor - prevMajor >= MAJOR_VERSION_JUMP_THRESHOLD) {
            findings.push({
                ...
                description: `'${context.packageName}' jumped from v${previousEntry.version} to v${version} ` +
...
function buildVersionTimeline(time) {
    const entries = [];
...
//# sourceMappingURL=publish-anomaly.js.map