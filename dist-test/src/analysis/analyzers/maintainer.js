...
export function analyzeMaintainerChanges(context) {
    const findings = [];
    ...
    const addedMaintainers = currentMaintainers.filter((m) => !previousMaintainers.some((p) => p.email === m.email || p.name === m.name));
    ...
    for (const added of addedMaintainers) {
        findings.push({
            ...
            description: `Version ${version} of '${packageName}' was published by or has a new maintainer ` +
