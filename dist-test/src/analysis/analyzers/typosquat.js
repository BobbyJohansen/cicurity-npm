...
const DATA_PATH = path.resolve(fileURLToPath(import.meta.url), '../../../../data/top-500-packages.json');
let cachedTop500 = null;
function loadTop500() {
    if (cachedTop500)
        return cachedTop500;
    try {
        cachedTop500 = _require(DATA_PATH);
        return cachedTop500;
    }
    catch {
...
export function levenshtein(a, b) {
    ...
    for (let i = 1; i <= a.length; i++) {
        ...
        for (let j = 1; j <= b.length; j++) {
            ...
            curr[j] = Math.min((curr[j - 1] ?? j) + 1, (prev[j] ?? i) + 1, (prev[j - 1] ?? i - 1) + cost);
        }
        [prev, curr] = [curr, prev];
    }
    return prev[b.length] ?? Math.max(a.length, b.length);
}
export function analyzeTyposquat(context) {
    const findings = [];
    ...
    for (const popular of top500) {
        ...
        if (dist > 0 && dist <= 2) {
            findings.push({
                ...
                description: `'${name}' is very similar to the popular package '${popular}' (Levenshtein distance ${dist}). ` +
