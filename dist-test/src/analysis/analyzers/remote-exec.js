...
export function analyzeRemoteExec(context) {
    const findings = [];
    for (const script of context.installScripts) {
        ...
        walkAst(ast, (node) => {
            ...
            const isDirectExec = node.type === 'CallExpression' &&
                node.callee?.type === 'Identifier' &&
                EXEC_METHODS.has(node.callee.name ?? '');
            // cp.exec('...'), child_process.execSync('...'), etc.
            const isMemberExec = node.type === 'CallExpression' &&
            ...
            if ((cmdLower.includes('powershell') || cmdLower.includes('pwsh')) &&
                ...
                findings.push({
                    ...
                    description: `The ${script.lifecycle} script runs PowerShell with an encoded command. ` +
            ...
            if (shellTarget) {
                findings.push({
                    ...
                    description: `The ${script.lifecycle} script executes a shell command that downloads content ` +
                ...
                findings.push({
                    ...
                    description: `The ${script.lifecycle} script executes '${downloadTool}' with a remote URL. ` +
                ...
                findings.push({
                    ...
                    description: `The ${script.lifecycle} script uses '${downloadTool}' in a shell command. ` +
 ...
 */
function extractCommandString(node) {
    if (!node)
        return null;
