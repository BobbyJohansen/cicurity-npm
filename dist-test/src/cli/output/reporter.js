 ...
 */
export function printPackageSummary(result) {
...
export function printReport(results) {
    ...
    if (blocked.length > 0) {
        process.stdout.write(colorize(ansi.redBold, `✗ Installation blocked. ${blocked.length} package(s) have critical security issues.\n`));
        for (const b of blocked) {
            process.stdout.write(`  To allowlist: ${colorize(ansi.cyan, `cicurity config allow ${b.packageName}@${b.version}`)}\n`);
        }
    }
...
function printFinding(finding, lifecycle) {
