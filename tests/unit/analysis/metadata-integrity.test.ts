...
import type { Packument, VersionMetadata } from '../../../src/registry/types.js';
...
function makePackument(v1Meta: Partial<VersionMetadata>, v2Meta: Partial<VersionMetadata>): Packument {
  return {
    name: 'test-pkg',
...
function makeContext(packument: Packument, version: string): AnalysisContext {
  return {
...
describe('analyzeMetadataIntegrity', () => {
  ...
  it('detects repository field removed as high', () => {
    ...
    (packument.versions['1.0.0'] as unknown as Record<string, unknown>)['repository'] = {
      type: 'git',
  ...
  it('detects author change as high', () => {
    ...
    (packument.versions['1.0.0'] as unknown as Record<string, unknown>)['author'] = { name: 'Alice', email: 'alice@example.com' };
    (packument.versions['1.0.1'] as unknown as Record<string, unknown>)['author'] = { name: 'attacker', email: 'attacker@evil.io' };
    const findings = analyzeMetadataIntegrity(makeContext(packument, '1.0.1'));
  ...
  it('does not flag when no changes between versions', () => {
    ...
    (packument.versions['1.0.0'] as unknown as Record<string, unknown>)['repository'] = repo;
    (packument.versions['1.0.1'] as unknown as Record<string, unknown>)['repository'] = repo;
    const findings = analyzeMetadataIntegrity(makeContext(packument, '1.0.1'));
