...
export async function runInstall(parsed: ParsedInstall): Promise<void> {
  ...
  if (!useJson) {
    process.stdout.write(
      `${colorize(ansi.bold, 'cicurity')} ${colorize(ansi.gray, `analyzing ${parsed.packages.length} package(s)...\n`)}`
    );
  }
  ...
  if (warned.length > 0 && !isCI) {
    const choice = await confirm(
      `${warned.length} package(s) have warnings. Proceed with install?`
    );
    if (choice === 'no') {
...
function runOriginalTool(parsed: ParsedInstall): void {
...
function preParseSpecifier(specifier: string): { name: string } {
