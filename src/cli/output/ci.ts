...
export function printJsonResults(results: PackageAnalysisResult[]): void {
  const output = {
...
export function printCiSummary(results: PackageAnalysisResult[]): void {
  ...
  if (blocked.length > 0) {
    ...
    for (const b of blocked) {
      for (const f of b.findings) {
        ...
        if (f.evidence) process.stderr.write(`    ${f.evidence.slice(0, 120)}\n`);
      }
    }
