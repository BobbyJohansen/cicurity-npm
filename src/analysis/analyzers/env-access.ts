...
export function analyzeEnvAccess(context: AnalysisContext): Finding[] {
      ...
      if (
        ...
        if (!processEnvAccessFound) {
          ...
          findings.push({
            ...
            description:
              `The ${script.lifecycle} script reads environment variables at install time. ` +
        ...
        if (parent && HIGH_VALUE_ENV_VARS.has(parent)) {
          findings.push({
            ...
            description:
              `The ${script.lifecycle} script specifically accesses \`process.env.${parent}\`, ` +
      ...
      if (
        ...
        if (HIGH_VALUE_ENV_VARS.has(keyName)) {
          findings.push({
            ...
            description:
              `The ${script.lifecycle} script accesses \`process.env['${keyName}']\`, ` +
              'a high-value credential commonly targeted by supply chain attackers.',
            evidence: sliceSource(src, node),
            file: fileRef,
            line: node.loc?.start.line,
