...
export function analyzeFilesystem(context: AnalysisContext): Finding[] {
    ...
    walkAst(ast, (node: AcornNode) => {
      ...
      if (isFsRequire(node)) {
        findings.push({
          ...
          description:
            `The ${script.lifecycle} script loads the 'fs' module. Filesystem access ` +
        ...
        if (FS_READ_METHODS.has(method)) {
          ...
          if (credentialPath) {
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script reads from a credential file path ('${pathArg}'). ` +
            ...
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script reads a shell config file ('${pathArg}'), ` +
            ...
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script reads from the filesystem using \`${method}()\`. ` +
        ...
        if (FS_WRITE_METHODS.has(method)) {
          ...
          if (shellPath) {
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script writes to a shell config file ('${pathArg}'). ` +
            ...
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script writes to a credential path ('${pathArg}'). ` +
            ...
            findings.push({
              ...
              title: `fs.${method}() called in install script`,
              description:
                `The ${script.lifecycle} script writes to the filesystem using \`${method}()\`. ` +
        ...
        if (FS_READ_METHODS.has(name) || FS_WRITE_METHODS.has(name)) {
          ...
          if (credentialPath || shellPath) {
            findings.push({
              ...
              description:
                `The ${script.lifecycle} script calls \`${name}()\` with a sensitive path ('${pathArg ?? ''}'). ` +
...
function isFsRequire(node: AcornNode): boolean {
  if (
    node.type === 'CallExpression' &&
    node.callee?.type === 'Identifier' &&
...
function extractStringArg(node: AcornNode | undefined): string | null {
  if (!node) return null;
...
function matchesCredentialPath(pathArg: string): string | null {
...
function matchesShellProfilePath(pathArg: string): string | null {
  const lower = pathArg.toLowerCase();
