...
export function analyzeChildProcess(context: AnalysisContext): Finding[] {
    ...
    walkAst(ast, (node: AcornNode) => {
      ...
      if (
        ...
        findings.push({
          ...
          description:
            `The ${script.lifecycle} script loads the 'child_process' module, enabling ` +
      ...
      if (
        ...
        findings.push({
          ...
          description:
            `The ${script.lifecycle} script calls \`${methodName}()\`, executing a ` +
      ...
      if (
        ...
        findings.push({
          ...
          description:
            `The ${script.lifecycle} script calls \`.${method}()\` to execute a command.`,
          evidence: sliceSource(src, node),
          file: fileRef,
          line: node.loc?.start.line,
        });
