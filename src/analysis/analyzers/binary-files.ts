...
async function readMagicBytes(filePath: string, count: number): Promise<Buffer | null> {
...
export async function analyzeBinaryFiles(context: AnalysisContext): Promise<Finding[]> {
  ...
  for (const relFile of context.allFiles) {
    ...
    if (BINARY_EXTENSIONS.has(ext)) {
      findings.push({
        ...
        description:
          `The package includes a prebuilt binary (${ext}) which cannot be statically analyzed for malicious code. ` +
      ...
      if (magic && magic.length >= 2) {
        ...
        if (detected) {
          findings.push({
            ...
            description:
              `The file '${relFile}' appears to be a ${detected} despite its extension. ` +
