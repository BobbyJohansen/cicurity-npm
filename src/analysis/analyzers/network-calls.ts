...
export function analyzeNetworkCalls(context: AnalysisContext): Finding[] {
...
function isRequireCall(node: AcornNode, modules: Set<string>): boolean {
  return (
...
function getRequireArg(node: AcornNode): string {
  return (node.arguments?.[0]?.value as string) ?? '';
}

function isDynamicImport(node: AcornNode, modules: Set<string>): boolean {
  return (
...
function getDynamicImportArg(node: AcornNode): string {
  return (node.source?.value as string) ?? '';
}

function isGlobalNetworkCall(node: AcornNode, globals: Set<string>): boolean {
...
function getGlobalName(node: AcornNode): string {
  return node.callee?.name ?? '';
}

function extractEvidence(source: string, node: AcornNode): string {
  if (!node.loc) return '';
