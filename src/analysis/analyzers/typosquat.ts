...
export function analyzeTyposquat(context: AnalysisContext): Finding[] {
  ...
  for (const popular of top500) {
    ...
    if (popular.startsWith('@') && !name.startsWith('@') && popularBareName.length > 0) {
      const scopeHijackDist = levenshtein(
        normalizeSeparators(name.toLowerCase()),
        normalizeSeparators(popularBareName.toLowerCase()),
      );
      if (scopeHijackDist === 0 || scopeHijackDist === 1) {
        findings.push({
          ...
          description:
            `'${name}' (unscoped) closely matches '${popular}' (scoped). ` +
