...
import type { VersionMetadata } from '../../registry/types.js';
...
export function analyzeMetadataIntegrity(context: AnalysisContext): Finding[] {
  ...
  const prevHooks = new Set(
    Object.keys(previous.scripts ?? {}).filter((k) => INSTALL_HOOKS.has(k)),
  );
  const currHooks = new Set(
    Object.keys(current.scripts ?? {}).filter((k) => INSTALL_HOOKS.has(k)),
  );
  ...
  for (const hook of currHooks) {
    if (!prevHooks.has(hook)) {
      ...
      findings.push({
        ...
        description:
          `Version ${version} of '${packageName}' adds a new '${hook}' script that was absent in ${previousVersion}. ` +
  ...
  if (prevRepo && !currRepo) {
    findings.push({
      ...
      description:
        `Version ${version} of '${packageName}' no longer has a repository URL, but ${previousVersion} pointed to '${prevRepo}'. ` +
  ...
  if (prevAuthor && currAuthor && prevAuthor !== currAuthor) {
    findings.push({
      ...
      description:
        `The author field of '${packageName}' changed between ${previousVersion} and ${version}. ` +
  ...
  if (prevHomepage && currHomepage && prevHomepage !== currHomepage) {
    findings.push({
      ...
      description:
        `The homepage URL domain of '${packageName}' changed between ${previousVersion} and ${version}. ` +
...
function getRepoUrl(meta: VersionMetadata): string | null {
  const repo = (meta as unknown as Record<string, unknown>)['repository'];
  if (!repo) return null;
...
function normalizeAuthor(meta: VersionMetadata): string | null {
  const author = (meta as unknown as Record<string, unknown>)['author'];
...
function getFieldString(meta: VersionMetadata, field: string): string | null {
  const val = (meta as unknown as Record<string, unknown>)[field];
