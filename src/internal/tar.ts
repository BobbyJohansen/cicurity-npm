...
interface TarHeader {
  ...
  typeflag: string;
  size: number;
  /** Absolute path where this entry should be extracted */
  dest: string;
}
 ...
 */
export async function extractTarball(tgzPath: string, destDir: string): Promise<string[]> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    piped.on('end', () => {
      try {
        ...
        let offset = 0;
        let paxName: string | null = null; // pax extended header override
        ...
        while (offset + BLOCK <= data.length) {
          ...
          if (destPath === null) continue; // path traversal attempt — skip
...
function parsePaxName(block: Buffer): string | null {
 ...
 */
export async function writeStream(readable: stream.Readable, destPath: string): Promise<void> {
